...
def MergeModelSet(context: 'Context', mname: str, A: z3.ExprRef, B_prioritized: z3.ExprRef) -> z3.ExprRef:
    ...
    return simplify(context.set_mk[mname](res_data, res_ids))
...
class Model:
    ...
    def set_sort(self):
        ...
        ret = z3.TupleSort(self.name + '_set', [data_sort, ids_sort])
        # print(ret)
        return ret
...
class Filteq(Expr):
    ...
    def eval(self, context: Context, sys: SystemState):
        ...
        newids = z3.Lambda([x], z3.If(z3.And([z3.IsMember(x, ids), selection_formula]),
                                      z3.BoolVal(True),
                                      z3.BoolVal(False)))
        return context.set_mk[self.model](newdata, newids)
...
class FilteqRel(Expr):
    ...
    def eval(self, context: Context, sys: SystemState):
        ...
        res = context.set_mk[from_name](res_data, res_ids)
        return res
...
class GFilteqRel(Expr):
    ...
    def eval(self, context: Context, sys: SystemState) -> z3.ExprRef:
        ...
        cur_model_state = context.set_mk[rels[-1].to_.name](cur_model_state_data, cur_model_state_ids)
        ...
        res_data = first_data
        res_model_state = context.set_mk[rels[0].from_](res_data, res_ids)
...
class Follow(Expr):
    ...
    def follow_oneone(self, context: Context, sys: SystemState, r: Relation):
        ...
        if self.direction == 'forward':
            ...
            return context.set_mk[r.to_.name](ret_data, ret_ids)
            ...
            return context.set_mk[r.from_.name](ret_data, ret_ids)
        else:
...
class First(Expr):
    ...
    def eval(self, context: Context, sys: SystemState) -> z3.ExprRef:
        ...
        context.add(z3.IsMember(ret_ref, ids))
        return obj
...
class Reverse(Expr):
    ...
    def eval(context, sys):
        ...
        return context.set_mk[self.model](data, ids)
