...
class BooleanField(Field):
    ...
    def to_internal_value(self, data):
        ...
        if isinstance(data, Sym):
            # Assume the input data is always a boolean value.
            # FIXME(ksqsf): NULL?
            data.type = bool
            _ = bool(data)  # trigger branch if necessary
            return data
        else:
            try:
...
class CharField(Field):
    ...
    def run_validation(self, data=empty):
        ...
        from Analyzer.symbolic import Sym
        if isinstance(data, Sym):
            from Analyzer.notify import add_cond
            from Verifier.checker import Unary, Binary, Str
            # data = str(data)
            add_cond(Unary('not', Binary('==', data.expr, Str(""))))
            return data
        else:
...
class IntegerField(Field):
    ...
    def to_internal_value(self, data):
        # BEGIN(ksqsf)
        from Analyzer.symbolic import Sym
        from Verifier.checker import Binary, Int
        if isinstance(data, Sym):
            data = int(data)
            from Analyzer.notify import add_cond
            if self.max_value is not None:
                add_cond(Binary('<=', data.expr, Int(self.max_value)))
                # add_cond(['<=', data.expr, self.max_value])
            if self.min_value is not None:
                add_cond(Binary('>=', data.expr, Int(self.max_value)))
                # add_cond(['>=', data.expr, self.min_value])
            return data
        else:
...
class FloatField(Field):
    ...
    def to_internal_value(self, data):
        # BEGIN(ksqsf)
        from Analyzer.symbolic import Sym
        from Verifier.checker import Binary, Real
        if isinstance(data, Sym):
            data = float(data)
            from Analyzer.notify import add_cond
            if self.max_value is not None:
                add_cond(Binary('>=', data.expr, Real(self.max_value)))
                # add_cond(['<=', data.expr, self.max_value])
            if self.min_value is not None:
                add_cond(Binary('<=', data.expr, Real(self.max_value)))
                # add_cond(['>=', data.expr, self.min_value])
    ...
    def to_internal_value(self, data):
        ...
        if isinstance(data, Sym):
            ...
            if self.max_value is not None:
                ...
                # add_cond(['<=', data.expr, self.max_value])
            if self.min_value is not None:
                add_cond(Binary('>=', data.expr, Real(self.min_value)))
                # add_cond(['>=', data.expr, self.min_value])
            return data
        else:
...
class ChoiceField(Field):
    ...
    def to_internal_value(self, data):
        ...
        from Analyzer.symbolic import Sym
        from Analyzer.notify import add_cond
        from Verifier.checker import Binary, Seq, Bool, Str
        if isinstance(data, Sym):
            if getattr(self, '_precise_choice', False):
                # Precise analysis: Use a loop to trigger the branch logic.
                data.type = str
                for choice in self.choice_strings_to_values.keys():
                    if data == choice:
                        # FIXME(ksqsf): Converted a symbolic value into a concrete one! This is probably wrong.
                        return self.choice_strings_to_values[choice]
                self.fail('invalid_choice', input=data)
            else:
                # Imprecise analysis.
                # ASSUME: choice types are uniform.
                data.type = type(next(iter(self.choice_strings_to_values.values())))
                choices = list(self.choice_strings_to_values.keys())
                cond = Bool(False)
                for choice in choices:
                    # FIXME: Str is debugging only
                    cond = Binary('||', Binary("==", data.expr, Str(choice)), cond)
                add_cond(cond)
                # add_cond(['in', data.expr, choices])
        else:
