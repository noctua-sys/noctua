...
from fake_constance import config as site_config
...
class SharedFromMePhotoSuperSimpleListViewSet2(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        qs = ThroughModel.objects.filter(photo_id__in=user_photos).order_by('photo__exif_timestamp')
...
class FavoritePhotoListViewset(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return Photo.objects.filter(favorited=True).filter(owner=self.request.user).order_by('-exif_timestamp')
...
#     def list(self, *args, **kwargs):
    ...
    def list(self, request):
        # queryset = Photo.objects.raw("SELECT image_hash, favorited, hidden, exif_timestamp from api_photo order by exif_timestamp desc")
        queryset = Photo.objects.filter(favorited=True).filter(owner=self.request.user).order_by('exif_timestamp')
        serializer = PhotoSuperSimpleSerializer(queryset, many=True)
        return Response({'results': serializer.data})
...
class HiddenPhotoListViewset(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return Photo.objects.filter(hidden=True).filter(owner=self.request.user).order_by('-exif_timestamp')
...
#     def list(self, *args, **kwargs):
    ...
    def list(self, request):
        # queryset = Photo.objects.raw("SELECT image_hash, favorited, hidden, exif_timestamp from api_photo order by exif_timestamp desc")
        queryset = Photo.objects.filter(hidden=True).filter(owner=self.request.user).order_by('exif_timestamp')
        serializer = PhotoSuperSimpleSerializer(queryset, many=True)
        return Response({'results': serializer.data})
...
class PublicPhotoListViewset(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if 'username' in self.request.query_params.keys():
            ...
            return Photo.objects.filter(public=True).filter(owner__username=username).order_by('-exif_timestamp')

        return Photo.objects.filter(public=True).order_by('-exif_timestamp')
...
#     queryset = Face.objects.all().select_related('person').order_by('id')
    ...
    def get_queryset(self):
        ...
        queryset = Face.objects.filter(photo__owner=self.request.user).order_by('id')
        return queryset
...
class FaceInferredListViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        # Todo: optimze query by only prefetching relevant models & fields
        queryset = Face.objects.filter(photo__owner=self.request.user).filter(person_label_is_inferred=True).order_by('id')
        return queryset
...
class FaceLabeledListViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        # Todo: optimze query by only prefetching relevant models & fields
        qs1 = Face.objects.filter(photo__owner=self.request.user).filter(person_label_is_inferred=False)
        qs2 = Face.objects.filter(person__name='unknown')
        queryset = qs1._union(qs2).order_by('id')
        return queryset
...
class FaceInferredViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return Face.objects.filter(photo__owner=self.request.user).filter(person_label_is_inferred=True).order_by('id')
...
class FaceLabeledViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return Face.objects.filter(photo__owner=self.request.user).filter(person_label_is_inferred=False).order_by('id')
...
#                   "gps_lat", 'people', "gps_lon", "photos")
        return AlbumAuto.objects.filter(owner=self.request.user).order_by('-timestamp')
...
class AlbumUserViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        qs1 = AlbumUser.objects.filter(owner=self.request.user)
        qs2 = AlbumUser.objects.filter(shared_to__exact=self.request.user.id)
        qs = qs1._union(qs2).distinct('id').order_by('-id')
        return qs
